import {AuthenticationPayload, AuthenticationReplyNonce, AuthenticationResponse, NonceData} from "./AuthenticationTypes";
...
class AuthenticationProvider extends ProviderBase {
    ...
    async authenticate(password: string): Promise<AuthenticationResponse> {
    ...
    async authenticateDevice(password: string): Promise<string> {
        ...
        return this.getNonce().then(noncePackage => {
            ...
            return this.prepareAuthPassword(password, noncePackage).then(reply => {
                ...
                return client.post("/authentication/device", reply, {withCredentials: true}).then(response => {
                    if (response.status === 200) {
                        return response.data;
                    }
    ...
    private async getNonce(): Promise<NonceData> {
        let client = this.createAxiosClient();
        return client.get("/authentication/nonce").then(response => {
            return response.data;
        }).catch(super.handleErrorResponse);
    }
    ...
    async forceSetPassword_temp(newPassword: string): Promise<boolean> {
        return this.getSalts().then(salts => {
            return this.prepareAuthPassword(newPassword, salts, true).then(reply => {
                return this.createAxiosClient().post("/authentication/todo_move_this", reply).then(response => {
...
export function createAuthenticationProvider(sharedSecret: Uint8Array) {
    return new AuthenticationProvider(sharedSecret);
}