...
import { EmojiSummary } from '../../Interacts';
import {
...
const Comment = ({ context, canReactDetails, commentData, onReply, isThread }: CommentProps) => {
...
export const CommentTeaser = ({ commentData }: { commentData: CommentReactionPreview }) => {
  ...
  return (
    <>
      <div className={''}>
        <div className="flex flex-row text-sm text-foreground text-opacity-50">
          <div className="flex flex-row">
            <p className="whitespace-pre-wrap">
              <span className="font-bold text-foreground text-opacity-70">
...
const CommentHead = ({
  ...
  const isOwner = getIdentity() === authorOdinId;
...
const CommentMedia = ({
  ...
  if (!imageUrl?.length)
    return (
      <div className="my-1 flex h-10 animate-pulse flex-row items-center justify-center bg-white text-sm text-foreground text-opacity-50">
        {t('loading')}
      </div>
    );
...
const CommentMeta = ({
  ...
  return (
    <div className="relative ml-[2.25rem] flex flex-row items-center px-2 pt-[2px] text-sm text-foreground text-opacity-20 dark:text-opacity-30">
      {canReact ? (
        <>
      ...
      />
      {canReact ? (
        <button className="ml-1 mr-2 text-button text-opacity-80 hover:underline" onClick={onReply}>
          {t('Reply')}
        </button>
      ) : null}
      {created ? (
  ...
  return (
    <>
      <div className="relative">
        ...
        <button
          className="ml-1 mr-1 text-button text-opacity-80 hover:underline"
          {...longPressEvent}
          onMouseEnter={() => setIsReact(true)}
          onMouseLeave={() => setIsReact(false)}
        >
