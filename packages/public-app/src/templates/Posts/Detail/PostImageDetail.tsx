...
const PostImageDetail = () => {
  ...
  return <PostImageDetailCard channel={blogData?.activeChannel} postFile={blogData?.activeBlog} />;
};
...
export const PostImageDetailCard = ({
  ...
  // Pop Current Index to get starting url
  ...
  const doSlide = (dir: 1 | -1) => {
    ...
    navigate(`${rootUrl}/${newIndex}`, { state: location.state, preventScrollReset: true });
  };

  const doClose = () => {
    navigate(state?.referrer || -1, { preventScrollReset: true });
  };
    ...
    <div className="relative z-50 bg-black lg:bg-transparent" role="dialog" aria-modal="true">
      ...
      <div className="inset-0 z-10 lg:fixed lg:overflow-y-auto">
        <div className="flex h-full min-h-screen flex-col lg:flex-row">
          ...
          <div className="flex-shrink-0 bg-background md:block lg:w-[25rem]">
            <div className="grid grid-flow-col grid-cols-[3rem_auto] gap-3 p-5 pb-0">
              ...
              <div className="flex max-w-lg flex-grow flex-col">
                <div className="mb-2 text-foreground text-opacity-60">
                  <h2 className="mr-4">
