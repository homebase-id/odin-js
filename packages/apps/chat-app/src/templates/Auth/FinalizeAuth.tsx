...
import { CHAT_ROOT_PATH } from '@homebase-id/common-app';
...
const AuthFinalize = () => {
  ...
  if (!identity || !public_key || !salt) return <Navigate to={`${CHAT_ROOT_PATH}/auth`} />;
  if (finalizeState === 'error')
    return <Navigate to={`${CHAT_ROOT_PATH}/auth?state=finalize-error`} />;
